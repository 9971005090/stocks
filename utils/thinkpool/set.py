...
__all__ = ['RUN_SAVE_SIGNAL_TODAY_BUY_TO_FIREBASE', 'RUN_SAVE_SIGNAL_CHOICE_DATE_BUY_STOCKS_TO_FIREBASE']
...
# 라씨 매수 JSON 파일에서 데이타를 긁어와, firebase에 저장
def _SAVE_SIGNAL_CHOICE_DATE_BUY_TO_FIREBASE(date, _TS):
    DELETE_TICKERS = []
    delete = False
    for _T in _TS:
        delete, _T['daum_stock_info'] = DAUM_GET.RUN_USE_CHECK_FOR_DELETE(_T['stockCode'].strip(), _T['stockName'], date)
    ...
    doc_id = date
    doc_data = []
    date_object = datetime.strptime(date, "%Y%m%d").isoformat()
    for stock in stocks:
        doc_data.append({
            'name': stock['name'],
            'code': stock['code'],
    ...
    FIREBASE.INFO['DB_OBJECT'].collection(FIREBASE.INFO['DB_COLLECTION_NAME']).document(doc_id).set({
        "stocks": doc_data,
        "created_at": date_object
    })
...
def RUN_SAVE_SIGNAL_CHOICE_DATE_BUY_TO_FIREBASE(now_date = None, root = None):
    if now_date is None:
        now_date = datetime.now().strftime("%Y%m%d")
    _TS = GET.RUN_READ_SIGNAL_TODAY_BUY_JSON(now_date, root)
    return _SAVE_SIGNAL_CHOICE_DATE_BUY_TO_FIREBASE(now_date, _TS)

def RUN_SAVE_SIGNAL_CHOICE_DATE_BUY_STOCKS_TO_FIREBASE(now_date = None, stocks = None):
    if now_date is None:
        now_date = datetime.now().strftime("%Y%m%d")
    if stocks is None:
        return False
    return _SAVE_SIGNAL_CHOICE_DATE_BUY_TO_FIREBASE(now_date, stocks)
